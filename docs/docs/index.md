...
│   │   ├── build_features.py
│   │   └── preprocess.py  <- an interface to preprocess seperate tables
│   │
...
[python-dotenv](https://github.com/theskumar/python-dotenv) to load up all the entries in this file as environment variables so they are accessible with `os.environ.get`. Here's an example snippet adapted from the `python-dotenv` documentation:
